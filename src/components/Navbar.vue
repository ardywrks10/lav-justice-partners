        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between h-16 items-center">
                ...
                <div class="hidden md:flex space-x-8">
                    <div v-for="item in menuItems" :key="item.id" class="relative group">
                        ...
                        <div v-if="item.subItems" 
                             class="absolute left-0 top-full w-40 bg-white border border-gray-100 shadow-xl rounded-md py-2 opacity-0 invisible translate-y-2 group-hover:opacity-100 group-hover:visible group-hover:translate-y-0 transition-all duration-300">
                            <a v-for="sub in item.subItems" 
                               :key="sub.name" 
                               :href="sub.link" 
                ...
                <div class="hidden lg:flex items-center">
                    <a href="/#contact" class="px-6 py-2.5 text-sm tracking-tighter font-bold rounded-sm bg-red-600 text-white hover:bg-red-800 transition-all shadow-sm">
                        Apply Now
                    </a>
                </div>
        ...
        <transition name="fade">
            <div v-show="isMobileMenuOpen" class="md:hidden bg-white border-t border-gray-100 shadow-2xl">
                <div class="px-6 py-6 space-y-1">
                    <div v-for="item in menuItems" :key="item.id" class="border-b border-slate-50 last:border-none">
                        ...
                        <template v-if="item.subItems">
                            <button @click="toggleSubmenu(item.id)" 
                                    class="flex items-center justify-between w-full py-4 text-sm font-bold text-slate-600 hover:text-red-600">
                                {{ item.name }}
                            ...
                            <div v-show="activeMobileSubmenu === item.id" 
                                class="pl-4 pb-4 space-y-3 transition-all duration-300">
                                <a v-for="sub in item.subItems" 
                                :key="sub.name" 
                                :href="sub.link" 
                    ...
                    <div class="pt-6">
                        <a href="/#contact" @click="isMobileMenuOpen = false" class="block w-full py-3 text-center bg-red-600 text-white font-bold text-sm rounded-sm">
                            Apply Now
                        </a>
                    </div>
                </div>
...
<script setup>
    ...
    const menuItems = [
        ...
        { id: 3, name: 'Lowongan Kerja', link: '/#lowongan'},
        { id: 4, name: 'Testimoni', link: '/#testimoni' },
        { id: 5, name: 'F.A.Q', link: '/#faq'},
        { id: 6, name: 'Contact', link: '/#contact'} // Memperbaiki typo #/contact menjadi /#contact
    ];
