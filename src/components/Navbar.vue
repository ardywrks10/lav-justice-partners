    ...
    <transition name="slide">
      <div v-show="isMobileMenuOpen" class="md:hidden fixed top-0 right-0 h-screen w-[320px] max-w-[85vw] bg-[#002D72] z-40 shadow-2xl flex flex-col justify-between pt-24 overflow-y-auto">
          ...
          <div v-for="item in menuItems" :key="item.id" class="border-b border-white/5 last:border-none">
            ...
            <template v-if="item.subItems">
              <button 
                ...
                class="flex items-center justify-between w-full py-4 text-lg font-bold text-white hover:text-[#F2A900] transition-colors" 
        ...
        <div class="p-6 border-t border-white/10 bg-[#001E4D]">
          <router-link to="/booking" @click="isMobileMenuOpen = false" class="flex justify-center items-center gap-2 w-full py-3.5 text-base font-black rounded-sm bg-[#F2A900] text-slate-950 hover:bg-[#d69500] transition-colors shadow-lg">
            Book Consultation
          </router-link>
        </div>
      </div>
